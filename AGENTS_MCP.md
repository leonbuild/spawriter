...
- **`execute`** — Playwright JS code with spawriter extensions (`page`, `context`, `state`, `navigate`, `screenshot`, `screenshotWithLabels`, `snapshot`, `interact`, `consoleLogs`, `networkLog`, `networkIntercept`, `dbg`, `editor`, `browserFetch`, `storage`, `emulation`, `performance`, `cssInspect`, `pageContent`, `singleSpa`, `clearCacheAndReload`, `getCDPSession`, `resetPlaywright`, `require`, `import`)
