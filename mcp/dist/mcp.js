...
const DEFAULT_IMAGE_PROFILE = { maxBytes: 5_000_000, maxLongEdge: 1568, format: 'webp', quality: 80 };
const TIER_LIMITS = { high: 5_000_000, medium: 5_000_000, low: 1_000_000 };
function resolveImageProfile(tier, modelHint) {
    const tierLimit = TIER_LIMITS[tier] ?? TIER_LIMITS.medium;
    if (modelHint) {
...
async function captureWithSizeGuarantee(session, profile) {
    const timeout = getCommandTimeout('Page.captureScreenshot');
    const captureParams = { format: profile.format };
    if (profile.format !== 'png') {
        captureParams.quality = profile.quality;
    }
    let result = await sendCdpCommand(session, 'Page.captureScreenshot', captureParams, timeout);
    ...
    for (let i = 0; i < MAX_COMPRESS_RETRIES; i++) {
        result = await sendCdpCommand(session, 'Page.captureScreenshot', { format: 'webp', quality, optimizeForSpeed: true }, timeout);
    ...
    result = await sendCdpCommand(session, 'Page.captureScreenshot', { format: 'webp', quality: 10, optimizeForSpeed: true }, timeout);
    return {
        data: result.data,
        mimeType: 'image/webp',
        originalSize,
...
function formatError(err) {
    ...
    if (err.recovery)
        parts.push(`Recovery: call "${err.recovery}" tool`);
    return parts.join('\n');
}
function formatAXTreeAsText(nodes, assignRefs = false, targetId) {
    const refCache = (assignRefs && targetId) ? getRefCache(targetId) : new Map();
    if (assignRefs)
        refCache.clear();
    const interactiveNodeIds = assignRefs ? new Set(getInteractiveElements(nodes).map(e => e.backendDOMNodeId)) : new Set();
...
const refCacheByTab = new Map();
function getRefCache(targetId) {
...
async function setTabTitlePrefix(session, prefix) {
    const reSrc = '^(?:🟢 |🟡 |🔴 |🔵 )+';
    const code = prefix
...
function stripRefPrefixes(text) {
...
function formatInteractiveSnapshot(elements) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const handleToolCall = async () => {
        ...
        if (name === 'connect_tab') {
            ...
            if (!result.success) {
                return { content: [{ type: 'text', text: formatError({ error: `Failed to connect tab: ${result.error || 'Unknown error'}`, hint: 'Chrome tab may not be available or extension may not be loaded', recovery: 'reset' }) }], isError: true };
            }
            await sleep(500);
            const targets = await getTargets();
        ...
        try {
            ...
            switch (name) {
                case 'screenshot': {
                    ...
                    const tier = args.quality || 'medium';
                    ...
                    if (!withLabels) {
                        ...
                        return {
                            content: [
                                ...
                                ...(capture.compressed ? [{ type: 'text', text: `Screenshot auto-compressed to fit ${(profile.effectiveLimit / 1_000_000).toFixed(0)}MB limit (${(capture.originalSize / 1024).toFixed(0)}KB → ${(capture.finalSize / 1024).toFixed(0)}KB)` }] : []),
                            ],
                        };
                    }
                    ...
                    return {
                        content: [
                            ...
                            ...(capture.compressed ? [{ type: 'text', text: `(auto-compressed to fit ${(profile.effectiveLimit / 1_000_000).toFixed(0)}MB limit)` }] : []),
                        ],
                    };
                }
                case 'accessibility_snapshot': {
                    ...
                    const interactiveOnly = args.interactive_only;
                    if (interactiveOnly) {
          ...
          return JSON.stringify(result);
                ...
                case 'storage': {
                    ...
                    switch (action) {
                        ...
                        case 'clear_storage': {
                            const origin = args.origin || await evaluateJs(session, 'window.location.origin');
                            const types = args.storage_types;
                            if (!types) {
              ...
              return JSON.stringify(results);
                ...
                case 'browser_fetch': {
                    ...
                    const maxSize = Math.max(1, Math.min(Number.isFinite(rawMaxSize) ? rawMaxSize : 10000, 100000));
        ...
        `;
                ...
                case 'trace': {
                    ...
                    const traceResult = await traceResp.json();
