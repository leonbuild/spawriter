 ...
 * Returns `{ preamble, returnExpr }` or null if no auto-return is possible.
 */
...
export function wrapCode(code: string): string {
  ...
  const multi = getLastExpressionReturn(code);
  if (multi !== null) return `(async () => { ${multi.preamble} return await (${multi.returnExpr}) })()`;
  return `(async () => { ${code} })()`;
}
...
export class PlaywrightExecutor {
  ...
  private async getCDPSession(page: Page): Promise<any> {
    if (this.cachedCdpSession?.page === page) {
      return this.cachedCdpSession.session;
    }
    try {
      const session = await page.context().newCDPSession(page);
      this.cachedCdpSession = { page, session };
      session.on('Debugger.scriptParsed', (params: any) => this.handleDebuggerEvent('Debugger.scriptParsed', params));
      session.on('Debugger.paused', (params: any) => this.handleDebuggerEvent('Debugger.paused', params));
      session.on('Debugger.resumed', () => this.handleDebuggerEvent('Debugger.resumed', {}));
      return session;
    } catch {
      return null;
    }
  ...
  private async sendCdp(cdpSession: any, method: string, params?: Record<string, unknown>, timeout?: number): Promise<unknown> {
    if (!cdpSession) throw new Error(`CDP session not available (required for ${method}). Try: session reset or relay --replace`);
  ...
  async execute(code: string, timeout = 30000, retryOnContextError = true): Promise<ExecuteResult> {
    ...
    try {
      ...
      code = code.replace(/(?<!\w)import\s*\(/g, '__dynamicImport__(');
      const autoReturnExpr = getAutoReturnExpression(code);
      const multiReturn = autoReturnExpr === null ? getLastExpressionReturn(code) : null;
      let wrappedCode: string;
      if (autoReturnExpr !== null) {
        wrappedCode = `(async () => { return await (${autoReturnExpr}) })()`;
      } else if (multiReturn !== null) {
        wrappedCode = `(async () => { ${multiReturn.preamble} return await (${multiReturn.returnExpr}) })()`;
      } else {
        wrappedCode = `(async () => { ${code} })()`;
      }
      const hasExplicitReturn = autoReturnExpr !== null || multiReturn !== null || /\breturn\b/.test(code);
  ...
  private buildVmGlobals(
    ...
    const relayCdp = async (method: string, params?: Record<string, unknown>, timeout?: number): Promise<unknown> => {
      ...
      throw new Error(`CDP session not available (required for ${method}). Try: session reset or relay --replace`);
    };
    ...
    const globals: Record<string, unknown> = {
      ...
      snapshot: async (options?: { search?: string; diff?: boolean; interactive_only?: boolean }) => {
        ...
        return '(accessibility snapshot requires CDP session — not available through relay connection. Try: session reset or relay --replace to recover)';
      },
      ...
      screenshotWithLabels: async (options?: { quality?: string; model?: string; max_elements?: number }) => {
        ...
        if (cdpSession) {
          ...
          const axResult = await sendCdpCmd('Accessibility.getFullAXTree', undefined, getCommandTimeout('Accessibility.getFullAXTree')) as any;
          const allInteractive = getInteractiveElements(axResult.nodes ?? []);
          const interactive = allInteractive.slice(0, maxElements);
      ...
      networkIntercept: {
        enable: async (urlPattern?: string) => {
          ...
          await page.route(urlPattern || '**/*', async (route) => {
            ...
            const rule = self.networkMonitor.findMatchingRule(req.url(), req.resourceType());
            if (!rule) {
            ...
            if (rule.mockStatus !== undefined || rule.mockBody !== undefined) {
              await route.fulfill({
        ...
        addRule: (rule: { url_pattern: string; mock_status?: number; mock_headers?: string | Record<string, string>; mock_body?: string; block?: boolean; resource_type?: string }) => {
          const newRule = self.networkMonitor.addInterceptRule({
            ...
            mockHeaders: rule.mock_headers
              ? (typeof rule.mock_headers === 'object' ? rule.mock_headers as unknown as Record<string, string> : JSON.parse(rule.mock_headers))
              : undefined,
      ...
      cssInspect: async (selector: string, properties?: string | string[]) => {
          ...
          try { el = document.querySelector(sel); } catch(e) {
            try {
              if (typeof CSS !== 'undefined' && CSS.escape) {
                var escaped = sel.replace(/#([^\\s.\\[]+)/g, function(_, id) { return '#' + CSS.escape(id); });
                el = document.querySelector(escaped);
              }
            } catch(e2) { return JSON.stringify({ error: 'Invalid selector: ' + sel + ' (' + e.message + ')' }); }
            if (!el) return JSON.stringify({ error: 'Invalid selector: ' + sel + ' (' + e.message + ')' });
          }
        ...
        const value = await evaluateJs(code);
        const parsed = typeof value === 'string' ? JSON.parse(value) : value;
        if (parsed?.error) return JSON.stringify({ error: true, message: parsed.error, selector });
      ...
      storage: async (action: string, options?: Record<string, unknown>) => {
        ...
        switch (action) {
          case 'get_cookies': {
            const filterDomain = opts.domain as string | undefined;
            const currentUrl = page.url();
            if (cdpSession) {
              const result = await sendCdpCmd('Network.getCookies', { urls: [currentUrl] }) as any;
              let cookies = result?.cookies || [];
              if (filterDomain) {
                cookies = cookies.filter((c: any) => c.domain === filterDomain || c.domain === '.' + filterDomain || filterDomain.endsWith(c.domain));
              }
              if (cookies.length === 0) return 'No cookies found for current page.';
              ...
              return `Cookies for ${currentUrl} (${cookies.length}):\n${lines.join('\n')}`;
            }
            let cookies = await page.context().cookies(currentUrl);
            if (filterDomain) {
              cookies = cookies.filter((c) => c.domain === filterDomain || c.domain === '.' + filterDomain || filterDomain.endsWith(c.domain));
            }
            if (cookies.length === 0) return 'No cookies found for current page.';
            ...
            if (cdpSession) {
              ...
              if (!params.url && !params.domain) {
                ...
                if (href && (href.startsWith('http://') || href.startsWith('https://'))) {
                  ...
                  if (!params.domain) return 'Error: set_cookie requires domain param — current page is not an http/https page.';
                }
              }
          ...
          case 'get_session_storage': {
            ...
            if (!currentHref || (!currentHref.startsWith('http://') && !currentHref.startsWith('https://'))) {
              return `Error: ${storageType} is not accessible on ${currentHref || 'this page'}. Navigate to an http/https page first.`;
            }
            try {
              ...
              return `Error: ${storageType} is not accessible — ${e.message || 'SecurityError'}. The page may be a restricted origin (about:blank, chrome-error, etc.).`;
            }
          }
      ...
      emulation: async (action: string, options?: Record<string, unknown>) => {
        const DEVICE_PRESETS: Record<string, { width: number; height: number; dpr: number; mobile: boolean; ua?: string }> = {
          ...
          'iphone-13': { width: 390, height: 844, dpr: 3, mobile: true },
          ...
          'galaxy-s21': { width: 360, height: 800, dpr: 3, mobile: true },
        };
        const opts = options || {};
        switch (action) {
          case 'set_device': {
            const deviceName = (opts.device as string)?.toLowerCase().replace(/\s+/g, '-');
            const preset = deviceName ? DEVICE_PRESETS[deviceName] : undefined;
            const width = (opts.width as number) || preset?.width || 375;
            const height = (opts.height as number) || preset?.height || 812;
            const dpr = (opts.device_scale_factor as number) || preset?.dpr || 1;
            const mobile = (opts.mobile as boolean) ?? preset?.mobile ?? false;
            await page.setViewportSize({ width, height });
            try {
              await relayCdp('Emulation.setDeviceMetricsOverride', { width, height, deviceScaleFactor: dpr, mobile });
            } catch { /* CDP fallback is optional */ }
            return `Device emulation: ${width}x${height} @${dpr}x${mobile ? ' (mobile)' : ''}${preset ? ` (${deviceName})` : ''}`;
          }
          case 'set_user_agent': {
          ...
          case 'set_timezone': {
            ...
            } catch { /* CDP path may fail through relay */ }
            await evaluateJs(`Object.defineProperty(Intl, 'DateTimeFormat', {
              value: new Proxy(Intl.DateTimeFormat, {
                construct(target, args) {
                  const opts = args[1] || {};
                  if (!opts.timeZone) opts.timeZone = ${JSON.stringify(tz)};
                  return new target(args[0], opts);
                },
                apply(target, thisArg, args) {
                  const opts = args[1] || {};
                  if (!opts.timeZone) opts.timeZone = ${JSON.stringify(tz)};
                  return target.call(thisArg, args[0], opts);
                }
              }),
              writable: true, configurable: true
            })`).catch(() => {});
            return `Timezone: ${tz} (applied via page override)`;
          }
          case 'set_locale': {
      ...
      performance: async (action?: string) => {
        ...
        switch (act) {
          ...
          case 'get_web_vitals': {
            const observerCode = `if (!window.__spawriter_lcp_obs) { window.__spawriter_lcp = null; window.__spawriter_cls = 0; window.__spawriter_inp = null; try { new PerformanceObserver(l => { for (const e of l.getEntries()) window.__spawriter_lcp = e.startTime; }).observe({type:'largest-contentful-paint',buffered:true}); } catch {} try { new PerformanceObserver(l => { for (const e of l.getEntries()) window.__spawriter_cls += e.value; }).observe({type:'layout-shift',buffered:true}); } catch {} try { new PerformanceObserver(l => { for (const e of l.getEntries()) { const d = e.processingEnd - e.startTime; if (window.__spawriter_inp === null || d > window.__spawriter_inp) window.__spawriter_inp = d; } }).observe({type:'event',buffered:true,durationThreshold:16}); } catch {} window.__spawriter_lcp_obs = true; } 'observers_active'`;
            await evaluateJs(observerCode);
            await new Promise(r => setTimeout(r, 300));
            const code = `JSON.stringify({ lcp: window.__spawriter_lcp, cls: window.__spawriter_cls, inp: window.__spawriter_inp, fcp: performance.getEntriesByName('first-contentful-paint')[0]?.startTime || null, ttfb: performance.getEntriesByType('navigation')[0]?.responseStart || null, domInteractive: performance.getEntriesByType('navigation')[0]?.domInteractive || null, domComplete: performance.getEntriesByType('navigation')[0]?.domComplete || null, loadTime: performance.getEntriesByType('navigation')[0]?.loadEventEnd || null })`;
            ...
            const fmt = (val: number | null, unit: string, good: number, poor: number) => {
              if (val === null || val === undefined) return '(not yet measured — interact with page or reload)';
            ...
            const lines = [
              ...
              `  INP: ${fmt(vitals.inp, 'ms', 200, 500)}`, `  FCP: ${vitals.fcp ? `${vitals.fcp.toFixed(0)}ms` : '(not available — requires fresh page load)'}`,
            ...
            return `Web Vitals:\n${lines.join('\n')}\n\n(Observers are auto-injected. LCP/CLS/INP update after page load or interaction. Call again for latest values.)`;
          }
          case 'get_memory': {
            try {
              await relayCdp('Performance.enable');
      ...
      editor: async (action: string, options?: Record<string, unknown>) => {
        ...
        switch (action) {
          ...
          case 'edit_source': {
            ...
            if (!scriptId || !content) return 'Error: edit requires scriptId and content';
            if (!self.debugger.enabled) { await relayCdp('Debugger.enable'); self.debugger.enabled = true; }
            try {
          ...
          case 'search':
          case 'search_source': {
            const search = (opts.search || opts.query) as string;
            if (!search) return 'Error: search requires search/query string';
            ...
            const code = `(() => {
              ...
              if (!text.trim()) {
                ...
                for (const f of iframes) {
                  try { const t = f.contentDocument?.body?.innerText?.trim(); if (t) { text += ' ' + t; } } catch {}
                }
              }
              return text || '(no visible text content)';
            })()`;
      ...
      clearCacheAndReload: async (options?: { clear?: string; reload?: boolean; origin?: string }) => {
        ...
        if (cdpSession) {
          const cdpStorageParts: string[] = [];
          if (clearTypes.has('local_storage')) cdpStorageParts.push('local_storage');
          if (clearTypes.has('cache_storage')) cdpStorageParts.push('cache_storage');
          if (clearTypes.has('indexeddb')) cdpStorageParts.push('indexeddb');
          if (clearTypes.has('service_workers')) cdpStorageParts.push('service_workers');
          if (cdpStorageParts.length > 0) {
            await sendCdpCmd('Storage.clearDataForOrigin', { origin, storageTypes: cdpStorageParts.join(',') });
            cleared.push(`${cdpStorageParts.join(', ')} (${origin})`);
          }
          if (clearTypes.has('session_storage')) {
            await evaluateJs('sessionStorage.clear()');
      ...
      resetPlaywright: async () => {
        ...
        return 'Playwright connection reset. Next execute call will reconnect.';
      },
    ...
    const dynamicImport = async (specifier: string) => {
      if (ALLOWED_MODULES.has(specifier) || ALLOWED_MODULES.has(`node:${specifier}`)) {
        const mod = self.sandboxedRequire(specifier);
        return { default: mod, ...mod };
      }
      throw new Error(
        `Module "${specifier}" is not allowed. ` +
        `Allowed modules: ${[...ALLOWED_MODULES].filter(m => !m.startsWith('node:')).join(', ')}`
      );
    };
    globals.import = dynamicImport;
    globals.__dynamicImport__ = dynamicImport;
